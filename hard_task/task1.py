...
@click.option(
    '--train_dest',
    prompt='Пункт назанчения',
    help='Пункт назначения поезда'
)
@click.option(
    '--number',
    prompt='Введите номер поезда',
    help='Номер поезда',
    type=int
)
@click.option(
    '--time',
    prompt='Время отправления',
    help='Время отправления'
)
def add_data(filename, train_dest, number, time):
...
def select_data(filename, time):
    if os.path.exists(filename):
        trains = load_trains(filename)
    else:
        trains = []
    select_train(f"select {time}", trains)
